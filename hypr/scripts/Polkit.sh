...
if systemctl --user is-enabled --quiet hyprpolkitagent.service 2>/dev/null || systemctl --user is-active --quiet hyprpolkitagent.service 2>/dev/null; then
  echo "hyprpolkitagent.service is enabled/active via systemd. Skipping manual start."
  exit 0
fi
